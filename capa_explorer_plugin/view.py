...
class CapaExplorerQtreeView(QtWidgets.QTreeView):
    ...
    def load_default_context_menu_actions(self, data):
        ...
        # If the row has an address value add the seek
        item = data[1]
        if item.data(1):
            default_actions.insert(0,("Seek to %s" % item.data(1), data, self.slot_seek_to))
    ...
    def load_function_context_menu_actions(self, data):
        ...
        if data[1].info.startswith('undefined.'):
            function_actions = (("Define function", data, self.slot_rename_function),)
        else:
